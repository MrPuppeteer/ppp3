...
string int_to_roman(int val) {
  if (val <= 0) return "NULLA";
  struct Roman_Pair {
    int v;
    string s;
  };
  vector<Roman_Pair> table = {
...
Token Token_stream::get() { // read characters from cin and compose a Token
  ...
  case 'I': case 'V': case 'X': case 'L': case 'C': case 'D': case 'M':
  {
    string s;
    s += ch;
    while (cin.get(ch) && char_to_roman(ch) != -1) s += ch;
    cin.putback(ch);
    return Token{number, double(roman_to_int(s))};
  }
...
void calculate() { // expression evaluation loop
  while (cin) try {
    ...
    if (res > 0 && res == floor(res) && res < 4000) {
      cout << result << int_to_roman(int(res)) << '\n';
    } else if (res == 0) {
      cout << result << "NULLA\n";
    } else {
      cout << result << res << " (Cannot be represented as Roman)\n";
    }
  } catch (exception& e) {
    cerr << e.what() << '\n'; // write error message
    clean_up_mess();
