/*
  ...
  Note: This calculator is based on 6.e.12.istream.cpp instead of 6.9.predefined_names.cpp
*/
...
constexpr char from_cmd = 'f';
constexpr char to_cmd = 't';

constexpr string fromkey = "from";
constexpr string tokey = "to";
...
Token Token_stream::get() {
  ...
  default:
    if (isalpha(ch) || ch == '_') {
      ...
      if (s == fromkey) return Token{from_cmd};
      if (s == tokey) return Token{to_cmd};
      return Token{name, s};
    }
...
void calculate(istream& is, ostream& os, string ofsname = "") {
  Token_stream ts{is};

  while (true) try {
    if (&is == &cin && &os == &cout) os << prompt;
    if (&is == &cin && &os != &cout) cout << "to " << ofsname << " " << prompt;
    ...
    if (t.kind == command) {
      ...
      if (t.name == helpkey) print_help(os);
      if (t.name == symkey) symbols.print(os);
      ts.ignore(print_token); // Ignore whatever comes after a command
    } else if (t.kind == to_cmd) {
      Token t2 = ts.get();
      if (t2.kind != name) throw runtime_error("filename expected after 'to'");

      ofstream ofs{t2.name};
      if (!ofs) throw runtime_error("could not create output file: " + t2.name);

      Token t3 = ts.get();
      if (t3.kind == from_cmd) {
        Token t4 = ts.get();
        if (t4.kind != name) throw runtime_error("filename expected after 'from'");

        ifstream ifs{t4.name};
        if (!ifs) throw runtime_error("could not open input file: " + t4.name);
        calculate(ifs, ofs);
      } else {
        ts.putback(t3);
        calculate(is, ofs, t2.name);
      }
    } else if (t.kind == from_cmd) {
      Token t2 = ts.get();
      if (t2.kind != name) throw runtime_error("filename expected after 'from'");

      ifstream ifs{t2.name};
      if (!ifs) throw runtime_error("could not open input file: " + t2.name);

      Token t3 = ts.get();
      if (t3.kind == to_cmd) {
        Token t4 = ts.get();
        if (t4.kind != name) throw runtime_error("filename expected after 'to'");

        ofstream ofs{t4.name};
        if (!ofs) throw runtime_error("could not create output file: " + t2.name);
        calculate(ifs, ofs);
      } else {
        ts.putback(t3);
